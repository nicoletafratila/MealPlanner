...
namespace MealPlanner.UI.Web.Services
{
    public class ShopService(HttpClient httpClient, IServiceProvider serviceProvider) : IShopService
    {
        ...
        {
            return await _httpClient.GetFromJsonAsync<ShopEditModel?>($"{_mealPlannerApiConfig?.Endpoints![ApiEndpointNames.ShopApi]}/edit/{id}");
        }
        ...
        public async Task<PagedList<ShopModel>?> SearchAsync(QueryParameters? queryParameters = null)
        {
            ...
            var response = await _httpClient.GetAsync(QueryHelpers.AddQueryString($"{_mealPlannerApiConfig?.Endpoints![ApiEndpointNames.ShopApi]}/search", query));
        ...
        public async Task<string?> AddAsync(ShopEditModel model)
        {
            ...
            var response = await _httpClient.PostAsync(_mealPlannerApiConfig?.Endpoints![ApiEndpointNames.ShopApi], modelJson);
        ...
        {
            var modelJson = new StringContent(JsonSerializer.Serialize(model), Encoding.UTF8, "application/json");
            var response = await _httpClient.PutAsync(_mealPlannerApiConfig?.Endpoints![ApiEndpointNames.ShopApi], modelJson);
            var result = Newtonsoft.Json.JsonConvert.DeserializeAnonymousType(await response.Content.ReadAsStringAsync(), new
            {
                Message = string.Empty
            });
        ...
        {
            var response = await _httpClient.DeleteAsync($"{_mealPlannerApiConfig?.Endpoints![ApiEndpointNames.ShopApi]}/{id}");
            var result = Newtonsoft.Json.JsonConvert.DeserializeAnonymousType(await response.Content.ReadAsStringAsync(), new
            {
                Message = string.Empty
            });
