...
namespace MealPlanner.UI.Web.Pages.RecipeBooks
{
    ...
    public partial class ProductStatistics
    {
        ...
        {
            Categories = await CategoryService.SearchAsync(QueryParameters!) ?? new PagedList<ProductCategoryModel>([], new Metadata { PageNumber = 1, PageSize = 1, TotalCount = 0 });
            ...
            var data = await StatisticsService.GetFavoriteProductsAsync(items) ?? [];
