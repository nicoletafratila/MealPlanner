using System.Net.Http.Json;using Common.Constants; using Common.Core; using Common.Http; using Common.Models; using Common.Pagination; using Common.Services; using Microsoft.Extensions.Logging; using RecipeBook.Shared.Constants; using RecipeBook.Shared.Models;

namespace RecipeBook.Services.Core
{
    public class ProductCategoryService(HttpClient httpClient, Common.Http.ITokenProvider tokenProvider, ILogger<ProductCategoryService> logger)
        : ServiceBase(httpClient, tokenProvider), IProductCategoryService
    {
        ...
        public async Task<CommandResponse?> AddAsync(ProductCategoryEditModel model, CancellationToken cancellationToken = default)
        {
            ...
            catch (Exception ex) { logger.LogError(ex, "ProductCategory AddAsync failed. Model {@Model}", model); throw; }
        }
        ...
        public async Task<CommandResponse?> UpdateAsync(ProductCategoryEditModel model, CancellationToken cancellationToken = default)
        {
            ...
            catch (Exception ex) { logger.LogError(ex, "ProductCategory UpdateAsync failed. Model {@Model}", model); throw; }
        }
        ...
        public async Task<CommandResponse?> DeleteAsync(int id, CancellationToken cancellationToken = default)
        {
            ...
            catch (Exception ex) { logger.LogError(ex, "ProductCategory DeleteAsync failed. Id {Id}", id); throw; }
        }
    }
}
