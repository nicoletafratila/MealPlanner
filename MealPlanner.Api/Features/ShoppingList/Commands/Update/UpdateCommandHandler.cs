...
{
    public class UpdateCommandHandler(IShoppingListRepository repository, IMapper mapper, ILogger<UpdateCommandHandler> logger) : IRequestHandler<UpdateCommand, UpdateCommandResponse>
    {
        ...
        public async Task<UpdateCommandResponse> Handle(UpdateCommand request, CancellationToken cancellationToken)
        {
            ...
            catch (Exception ex)
            {
                ...
                return new UpdateCommandResponse { Message = "An error occurred when saving the meal plan." };
            }
        }
    }
}
