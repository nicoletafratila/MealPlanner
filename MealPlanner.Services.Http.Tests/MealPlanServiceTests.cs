...
namespace MealPlanner.Services.Http.Tests
{
    ...
    public class MealPlanServiceTests
    {
        ...
        public void GetEditAsync_Throws_OnNonSuccessStatusCode()
        {
            ...
            // Act & Assert
            Assert.ThrowsAsync<HttpRequestException>(async () => await service.GetEditAsync(Guid.NewGuid()));
            mockHttp.VerifyNoOutstandingExpectation();
        }
        ...
        public async Task GetCurrentAsync_ReturnsFirstItem_WhenMealPlanFound()
        {
            ...
            var paged = new PagedList<MealPlanModel>(
                [expected, new MealPlanModel { Id = Guid.NewGuid() }],
        ...
        {
            var userAResult = new PagedList<MealPlanModel>([new MealPlanModel()], new Metadata { PageNumber = 1, PageSize = 10, TotalCount = 1 });
            var userBResult = new PagedList<MealPlanModel>([new MealPlanModel(), new MealPlanModel()], new Metadata { PageNumber = 1, PageSize = 10, TotalCount = 2 });
        ...
        {
            // Arrange
            var model = new MealPlanEditModel { Id = Guid.NewGuid() };
