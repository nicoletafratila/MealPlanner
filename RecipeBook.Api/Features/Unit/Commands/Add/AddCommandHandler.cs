        ...
        ILogger<AddCommandHandler> logger) : IRequestHandler<AddCommand, CommandResponse?>
    {
        ...
        public async Task<CommandResponse?> Handle(AddCommand request, CancellationToken cancellationToken)
        {
            ...
            if (request.Model is null)
                throw new ArgumentNullException(nameof(request), UnitMessages.ModelCannotBeNull);
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(newName))
                {
                    ...
                    if (duplicate != null)
                    {
                        return CommandResponse.Failed(UnitMessages.UnitAlreadyExists);
                    }
                }

                var mapped = _mapper.Map<Data.Entities.Unit>(request.Model);
