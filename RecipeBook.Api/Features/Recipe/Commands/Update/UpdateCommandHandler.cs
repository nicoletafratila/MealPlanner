...
{
    public class UpdateCommandHandler(IRecipeRepository repository, IMapper mapper, ILogger<UpdateCommandHandler> logger) : IRequestHandler<UpdateCommand, CommandResponse>
    {
        public async Task<CommandResponse> Handle(UpdateCommand request, CancellationToken cancellationToken)
        {
            try
            {
                var existingItem = await repository.GetByIdIncludeIngredientsAsync(request.Model?.Id);
