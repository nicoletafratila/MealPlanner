using System.Reflection;using Blazored.Modal; using Bunit; using Common.Models; using Common.Pagination; using Common.UI; using MealPlanner.Services.Http; using MealPlanner.Shared.Models; using MealPlanner.UI.Web.Pages.MealPlans; using Microsoft.AspNetCore.Components; using Microsoft.Extensions.DependencyInjection; using Moq; using RecipeBook.Services.Core; using RecipeBook.Shared.Models;
...
namespace MealPlanner.UI.Web.Tests.Pages.MealPlans
{
    ...
    public class ShoppingListEditTests
    {
        ...
        public void OnInitializedAsync_WithNonZeroId_LoadsShoppingList()
        {
            ...
            _shopServiceMock
                .Setup(s => s.GetEditAsync(1, CancellationToken.None))
            ...
            _shoppingListServiceMock.Verify(s => s.GetEditAsync(5, CancellationToken.None), Times.Once);
        }
        ...
        public async Task SaveCoreAsync_UpdatesShoppingList_WhenIdIsNonZero()
        {
            ...
            // Assert
            _shopServiceMock.Verify(s => s.GetEditAsync(1, CancellationToken.None), Times.Once);
            _shoppingListServiceMock.Verify(s => s.GetEditAsync(5, CancellationToken.None), Times.Once);
            _shoppingListServiceMock.Verify(
                s => s.UpdateAsync(It.Is<ShoppingListEditModel>(m => m.Id == 5), CancellationToken.None),
                Times.Once);
