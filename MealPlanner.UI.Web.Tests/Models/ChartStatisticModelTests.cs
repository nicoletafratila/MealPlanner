...
namespace MealPlanner.UI.Web.Tests.Models
{
    [TestFixture]
    public class ChartStatisticModelTests
    {
        [Test]
        public void Constructor_FromStatisticModel_CopiesAllCoreFields()
        {
            // Arrange
            var source = new StatisticModel
            {
            ...
            // Act
            var chart = new ChartStatisticModel(source);

            // Assert
            using (Assert.EnterMultipleScope())
            {
                Assert.That(chart.Title, Is.EqualTo(source.Title));
                Assert.That(chart.Label, Is.EqualTo(source.Label));
                Assert.That(chart.Index, Is.EqualTo(source.Index));
                Assert.That(chart.IsSelected, Is.EqualTo(source.IsSelected));
                Assert.That(chart.Data, Is.SameAs(source.Data));
            }
        }
        ...
        {
            Assert.Throws<ArgumentNullException>(() => _ = new ChartStatisticModel(null!));
        }
        ...
        {
            // Arrange
            var model = new ChartStatisticModel
            {
                Title = "Favorite Recipes",
                Label = "Recipes",
                Data = new Dictionary<string, double?>
