...
namespace RecipeBook.Api.Tests.Features.RecipeCategory.Queries.GetEdit
{
    ...
    public class GetEditQueryHandlerTests
    {
        ...
        public async Task Handle_EntityFound_ReturnsMappedModel()
        {
            // Arrange
            const int id = 5;
            var entity = new RecipeBook.Data.Entities.RecipeCategory
            {
                Id = id,
                Name = "Breakfast",
                DisplaySequence = 1
        ...
        public async Task Handle_EntityNotFound_ReturnsEmptyModelWithId()
        {
            ...
            _repoMock.Verify(r => r.GetByIdAsync(id, CancellationToken.None), Times.Once);
            _mapperMock.Verify(m => m.Map<RecipeCategoryEditModel>(It.IsAny<RecipeBook.Data.Entities.RecipeCategory>()), Times.Never);
        }
