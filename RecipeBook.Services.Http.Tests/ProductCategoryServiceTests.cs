...
namespace RecipeBook.Services.Http.Tests
{
    ...
    public class ProductCategoryServiceTests
    {
            ...
            IMemoryCache? cache = null)
        {
            ...
            return new ProductCategoryService(httpClient, tokenProvider.Object, cache ?? new MemoryCache(new MemoryCacheOptions()), logger);
        }
        ...
        {
            var paged = new PagedList<ProductCategoryModel>([new ProductCategoryModel()], new Metadata { PageNumber = 1, PageSize = 10, TotalCount = 1 });

            var mockHttp = new MockHttpMessageHandler();
            mockHttp.Expect(HttpMethod.Get, $"{BaseAddress}{ProductCategoryPath}/search*")
                .Respond("application/json", JsonSerializer.Serialize(paged, JsonOptions));
        ...
        public async Task AddAsync_InvalidatesCache_NextSearchHitsHttp()
        {
            ...
            mockHttp.Expect(HttpMethod.Get, $"{BaseAddress}{ProductCategoryPath}/search*")
                .Respond("application/json", JsonSerializer.Serialize(paged, JsonOptions));
            mockHttp.Expect(HttpMethod.Post, $"{BaseAddress}{ProductCategoryPath}")
            ...
            await service.SearchAsync();
            await service.AddAsync(new ProductCategoryEditModel { Id = Guid.NewGuid() });
            await service.SearchAsync();
        ...
        public async Task UpdateAsync_InvalidatesCache_NextSearchHitsHttp()
        {
            ...
            mockHttp.Expect(HttpMethod.Get, $"{BaseAddress}{ProductCategoryPath}/search*")
                .Respond("application/json", JsonSerializer.Serialize(paged, JsonOptions));
            mockHttp.Expect(HttpMethod.Put, $"{BaseAddress}{ProductCategoryPath}")
            ...
            await service.SearchAsync();
            await service.UpdateAsync(new ProductCategoryEditModel { Id = Guid.NewGuid() });
            await service.SearchAsync();
        ...
        public async Task DeleteAsync_InvalidatesCache_NextSearchHitsHttp()
        {
            ...
            mockHttp.Expect(HttpMethod.Get, $"{BaseAddress}{ProductCategoryPath}/search*")
                .Respond("application/json", JsonSerializer.Serialize(paged, JsonOptions));
            mockHttp.Expect(HttpMethod.Delete, $"{BaseAddress}{ProductCategoryPath}*")
        ...
        {
            var userAResult = new PagedList<ProductCategoryModel>([new ProductCategoryModel()], new Metadata { PageNumber = 1, PageSize = 10, TotalCount = 1 });
            var userBResult = new PagedList<ProductCategoryModel>([new ProductCategoryModel(), new ProductCategoryModel()], new Metadata { PageNumber = 1, PageSize = 10, TotalCount = 2 });
