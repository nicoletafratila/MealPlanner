...
namespace MealPlanner.UI.Mobile.ViewModels.RecipeBook
{
    public partial class UnitsOverviewViewModel(UnitService unitService) : BaseViewModel
    {
        ...
        private async Task LoadAsync()
        {
            ...
            try
            {
                var result = await unitService.SearchAsync(new QueryParameters<UnitModel> { PageSize = 200 });
        ...
        {
            var (success, error) = await unitService.DeleteAsync(u.Id);
            if (success) Units.Remove(u);
            else SetError(error);
        }
    }
}
