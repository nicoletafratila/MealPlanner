...
{
    public partial class MealPlansOverviewViewModel(MealPlanService mealPlanService) : BaseViewModel
    {
        ...
        {
            var (success, error) = await mealPlanService.DeleteAsync(mp.Id);
            if (success) MealPlans.Remove(mp);
            else SetError(error);
        }
    }
}
