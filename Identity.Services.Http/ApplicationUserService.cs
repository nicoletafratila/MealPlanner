...
namespace Identity.Services.Http
{
        ...
        : ServiceBase(httpClient, tokenProvider), IApplicationUserService
    {
        ...
        public async Task<CommandResponse?> UnlockAsync(string userId, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                return await PostAsync($"{_controller}/{IdentityControllers.UnlockRoute}", new { UserId = userId }, cancellationToken);
            }
            catch (HttpRequestException ex)
            {
                logger.LogWarning(ex, "UnlockAsync failed.");
                return CommandResponse.Failed(ex.Message ?? Resources.ApplicationUserServiceMessages.UnlockUserFailed);
            }
        ...
        public async Task<CommandResponse?> UpdateAsync(ApplicationUserEditModel model, CancellationToken cancellationToken = default)
        {
            ...
            catch (HttpRequestException ex)
            {
                ...
                return CommandResponse.Failed(ex.Message ?? Resources.ApplicationUserServiceMessages.UpdateUserFailed);
            }
            catch (JsonException ex)
            {
