...
{
    public class UserService(HttpClient httpClient, ITokenProvider tokenProvider, ILogger<UserService> logger)
        : ServiceBase(httpClient, tokenProvider)
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            {
                logger.LogError(ex, "{ServiceName} SearchAsync failed", nameof(UserService));
                return null;
            }
        }
        ...
        public async Task<(bool Success, string? Error)> UpdateAsync(ApplicationUserEditModel model, CancellationToken cancellationToken = default)
        {
            ...
            {
                logger.LogError(ex, "{ServiceName} UpdateAsync failed for model {@Model}", nameof(UserService), model);
        ...
        public async Task<(bool Success, string? Error)> UnlockAsync(string userId, CancellationToken cancellationToken = default)
        {
            ...
            {
                logger.LogError(ex, "{ServiceName} UnlockAsync failed for userId {UserId}", nameof(UserService), userId);
                return (false, ex.Message);
            }
        }
    }
